...
class ReadmeUpdater {
    ...
    getLatestCommit() {
        try {
            ...
            const commitDate = execSync('git log -1 --pretty=%ad --date=short', { encoding: 'utf-8' }).trim();
    ...
    analyzeCommit(commit) {
        ...
        // Feature detection
        if (message.includes('add') || message.includes('new') || message.includes('implement') || message.includes('feature')) {
            if (message.includes('json') || message.includes('format') || message.includes('output')) {
        ...
        // Fix detection
        if (message.includes('fix') || message.includes('bug') || message.includes('error') || message.includes('issue')) {
        ...
        // Dependency changes
        if (files.some(f => f.includes('package.json') || f.includes('bun.lock'))) {
            changes.dependencies.push('Updated dependencies');
        }

        // Breaking changes
        if (message.includes('breaking') || message.includes('break') || message.includes('major')) {
...
`;
            ...
            if (featuresIndex !== -1) {
                readme = readme.slice(0, featuresIndex) + changelogSection + readme.slice(featuresIndex);
                updated = true;
            }
        }
...
`;
            ...
            if (existingEntries.length > 0) {
                changelogContent += existingEntries.map(entry => '### ' + entry).join('');
            }
